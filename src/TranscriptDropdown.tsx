...
export default class TranscriptDropdown extends React.Component<ITranscriptDropdownProps, {}>
{
    public render()
    {
        ...
        const canonicalTranscriptId = canonicalTranscript.result && canonicalTranscript.result.transcriptId;

        if (!showDropDown) {
            return <span />;
        } else if (showOnlyAnnotatedTranscriptsInDropdown) {
            const isLoading = transcriptsWithProteinLength.isPending ||
                transcriptsWithAnnotations.isPending ||
                canonicalTranscript.isPending;
            const requiredData = indexedVariantAnnotations.result &&
            ...
            const isLoading = transcriptsWithProteinLength.isPending || canonicalTranscript.isPending;
            const requiredData = transcriptsWithProteinLength.result &&
    ...
    private loadingIndicator(isLoading: boolean) {
        return isLoading ? (this.props.loadingIndicator || <i className="fa fa-spinner fa-pulse fa-2x" />) : null;
    }
}
